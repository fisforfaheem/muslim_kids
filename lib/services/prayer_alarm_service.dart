...
class PrayerAlarmService {
  ...
  Future<Position?> getCurrentLocation() async {
    ...
    try {
      ...
      if (_lastKnownLocation != null) {
        ...
        // Set a more user-friendly location name
  ...
  // Set a more user-friendly location name based on coordinates
  void _setUserFriendlyLocationName(
    Position position, {
    bool isLastKnown = false,
  }) {
    // For now, we'll use a simple approach with major cities
    // In a production app, you might want to use reverse geocoding

    // Islamabad coordinates
    const double islamabadLat = 33.6844;
    const double islamabadLng = 73.0479;
    ...
    // Lahore coordinates
    const double lahoreLat = 31.5204;
    const double lahoreLng = 74.3587;

    // Calculate distance to known cities
    double distToIslamabad = Geolocator.distanceBetween(
      position.latitude,
      position.longitude,
      islamabadLat,
      islamabadLng,
    ...
    double distToKarachi = Geolocator.distanceBetween(
      position.latitude,
      position.longitude,
      karachiLat,
      karachiLng,
    ...
    double distToLahore = Geolocator.distanceBetween(
      position.latitude,
      position.longitude,
      lahoreLat,
      lahoreLng,
    ...
    if (distToIslamabad < closestDist) {
      closestDist = distToIslamabad;
      cityName = "Islamabad";
    }

    if (distToKarachi < closestDist) {
      closestDist = distToKarachi;
      cityName = "Karachi";
    }

    if (distToLahore < closestDist) {
      closestDist = distToLahore;
      cityName = "Lahore";
    }

    // If we're within 50km of a known city, use that name
    if (closestDist < 50000) {
      _locationName =
          isLastKnown ? "Near $cityName (last known)" : "Near $cityName";
    } else {
      // Otherwise use coordinates but in a more friendly format
      _locationName = isLastKnown ? "Last known location" : "Current location";
    }
  }
  ...
  Future<bool> _canAccessFirestore() async {
    try {
      // Try to access a public collection that should be readable by all users
